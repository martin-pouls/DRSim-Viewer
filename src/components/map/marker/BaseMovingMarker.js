...
L.BaseMovingMarker = L.Marker.extend({
    options: {
        bearingAngle: 0,
        rotationOrigin: ''
    },
    ...
    _applyRotation: function () {
        if (this.options.bearingAngle) {
            ...
            if (this.options.bearingAngle > 90 && this.options.bearingAngle < 270) {
                this._icon.style[L.DomUtil.TRANSFORM] += ' scale(1, -1)'
            }
        }
    },
...
L.baseMovingMarker = function (latlng, options) {
    return new L.BaseMovingMarker(latlng, options)
}

export var BaseMovingMarker = L.BaseMovingMarker